...
RSpec.describe Transaction, type: :model do
  ...
  describe '#above_daily_limit?' do
    ...
    let(:transaction) { create(:transaction, user_id: user.id, transaction_date: transaction_date, transaction_amount: 600.00) }

    it 'returns true if the daily total exceeds the limit' do
      create(:transaction, user_id: user.id, transaction_date: transaction_date.beginning_of_day + 2.hours, transaction_amount: 500.00)

      expect(transaction.above_daily_limit?).to be true
    end

    it 'returns false if the daily total does not exceed the limit' do
      create(:transaction, user_id: user.id, transaction_date: transaction_date.beginning_of_day + 2.hours, transaction_amount: 300.00)
  ...
  describe '#unusual_time_of_day?' do
    it 'returns true if the transaction is made before 6 AM' do
      transaction = build(:transaction, transaction_date: Time.now.change(hour: 5))
      expect(transaction.unusual_time_of_day?).to be true
    end

    it 'returns true if the transaction is made after 8 PM' do
      transaction = build(:transaction, transaction_date: Time.now.change(hour: 21))
      expect(transaction.unusual_time_of_day?).to be true
    end

    it 'returns false if the transaction is made between 6 AM and 8 PM' do
      transaction = build(:transaction, transaction_date: Time.now.change(hour: 14))
  ...
  describe '#frequent_transactions?' do
    ...
    it 'returns true if there are more than 3 transactions in the last hour' do
      create_list(:transaction, 4, user_id: user.id, transaction_date: 30.minutes.ago)
      transaction = build(:transaction, user_id: user.id)
    ...
    it 'returns false if there are 3 or fewer transactions in the last hour' do
      create_list(:transaction, 3, user_id: user.id, transaction_date: 30.minutes.ago)
      transaction = build(:transaction, user_id: user.id)
  ...
  describe '#previous_fraud_chargeback?' do
    ...
    it 'returns true if there is a previous chargeback for the user' do
      create(:transaction, user_id: user.id, has_cbk: true)
      transaction = build(:transaction, user_id: user.id)
    ...
    it 'returns false if there are no previous chargebacks' do
      transaction = build(:transaction, user_id: user.id, card_number: card_number)
